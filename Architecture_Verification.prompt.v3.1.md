...
# 🏗️ Sys.Kernel Architecture Verification Prompt (Level 3.2)
...
|:--|:--|:--|
| **Kernel.Sdk** | `J.P.Hahnl.Kernel.Sdk` | Build-time metadata layer. Holds compiler-generated definitions (T4/MSBuild), constants, attributes, and base exceptions. Represents the interface to the framework. |
| **Kernel.Core** | `J.P.Hahnl.Kernel.Core` | Runtime primitives layer. Depends on Sdk. Extends SDK definitions with functional logic, immutable value objects, and runtime-safe constructs. |
| **Kernel.Runtime** | `J.P.Hahnl.Kernel.Runtime` | Execution layer. Hosts runtime logic, configuration management, service lifetimes, and dependency injection. Depends on Core. |
| **Kernel.Diagnostics** | `J.P.Hahnl.Kernel.Diagnostics` | Telemetry and logging layer. Depends on Runtime. Provides structured logging, EventSource tracing, and OpenTelemetry integration. |
| **Kernel.Development** | `J.P.Hahnl.Kernel.Development` | Developer tools and analyzers. Depends on all internal assemblies but excluded from release packages. |
| **Kernel.Tests** | `J.P.Hahnl.Kernel.Tests` | Validation and architectural verification layer. Depends on all others but is not depended upon. |
...
1. **SDK Foundation**  
   - `J.P.Hahnl.Kernel.Sdk` contains all constants, attributes, identifiers, and base exceptions.  
...
public void Kernel_Core_ShouldOnlyDependOn_Sdk()
{
    var result = Types.InAssembly("J.P.Hahnl.Kernel.Core")
        .That().ResideInNamespace("J.P.Hahnl.Kernel.Core")
...
Types.InAssemblies(AppDomain.CurrentDomain.GetAssemblies())
    ...
    .Should().OnlyDependOn("J.P.Hahnl.Kernel.Sdk")
    .GetResult();
```
...
- Add analyzer rule: "Attributes must originate from `J.P.Hahnl.Kernel.Sdk`."
